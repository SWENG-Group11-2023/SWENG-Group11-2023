import React from 'react';
import {
  MDBCard,
  MDBCardBody,
  MDBCardTitle,
  MDBCardText,
  MDBBtn,
  MDBRow,
  MDBCol
} from 'mdb-react-ui-kit';

export default function App() {
  return (
    <MDBRow className='row-cols-1 row-cols-md-3 g-4'>
      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Austeja Pakulyte</MDBCardTitle>
            <MDBCardText>
              3rd Year - Intergrated Computer Science <br></br>
              Management & Frontend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/pakulyta'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Constantin Pusch</MDBCardTitle>
            <MDBCardText>
              3rd Year - Computer Science & Business <br></br>
              Management & Backend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/cpusch'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Kevin Morley</MDBCardTitle>
            <MDBCardText>
              3rd Year - Intergrated Computer Science <br></br>
              Product Owner
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/k-e-v-i-n17'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Andrew Furey</MDBCardTitle>
            <MDBCardText>
              2nd Year - Intergrated Computer Science <br></br>
              Backend Development 
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/anfurey'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Anthony Oisin Gavril </MDBCardTitle>
            <MDBCardText>
              2nd Year - Integrated Computer Science <br></br>
              Frontend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/onion-mints'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Isobel Radford-Dodd</MDBCardTitle>
            <MDBCardText>
              2nd Year - Computer Science & Business <br></br>
              Frontend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/IsobelRadford-Dodd'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Luke Barry</MDBCardTitle>
            <MDBCardText>
              2nd Year - Computer Science & Business <br></br>
              Frontend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/Luke-Barry'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Minghim Foun</MDBCardTitle>
            <MDBCardText>
              2nd Year - Integrated Computer Science <br></br>
              Backend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/OscarFoun'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

      <MDBCol>
        <MDBCard>
          <MDBCardBody>
            <MDBCardTitle>Samuel Forster</MDBCardTitle>
            <MDBCardText>
              2nd Year - Intergrated Computer Science <br></br>
              Backend development
            </MDBCardText>
            <div class="text-center">
            <MDBBtn style={{ backgroundColor: 'skyblue' }} target="_blank" href='https://github.com/SamPFF'>Github</MDBBtn>
            </div>
          </MDBCardBody>
        </MDBCard>
      </MDBCol>

    </MDBRow>
  );
}